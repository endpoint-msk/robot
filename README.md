...
- Кнопка миниаппа появляется рядом с полем ввода в личке с ботом (ставится автоматически через API), под каждым списком `/inside` в чатах («Хочу прийти») и в `/start`-меню.
- **Тема** — системная по умолчанию (следует теме Telegram), в настройках миниаппа можно закрепить светлую или тёмную. Настройки открыты и гостям — им там доступна только тема.
- В подтверждённом визите есть кнопка **«Добавить в календарь»** — сервер отдаёт `.ics` по подписанной ссылке.
- Для userId из `DEV_USER_IDS` в миниаппе есть дев-меню: переключатель перспективы «резидент ↔ гость», создание фейковых заявок, а также правка и удаление любых заявок.
...
Переменные окружения — в `.env.example`. Обязательны `API_ID`, `API_HASH`, `BOT_TOKEN`, `ALLOWED_CHATS`. Опционально: `PRINTER_URL`/`PRINTER_AUTH` (иначе `/printer` выключен), `KEENETIC_URL`/`KEENETIC_LOGIN`/`KEENETIC_PASSWORD` (иначе авто-отметки по MAC выключены), `WEBAPP_URL`/`WEBAPP_HOST`/`WEBAPP_PORT`/`HOSTING_TZ_OFFSET_MINUTES` (иначе миниапп хостинга выключен), `DATA_FILE`, форвардинг и live-чат.
...
- `webapp/` — фронт миниаппа (vanilla JS, дизайн-система iOS: светлая и тёмная темы).
