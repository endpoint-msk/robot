...
- **Релиз** — воркфлоу `release.yml` триггерится пушем тега `vX.Y.Z`. Шаг «Ensure tag is on main» гейтит: `git merge-base --is-ancestor $GITHUB_SHA origin/main` — тег на dev игнорируется (релизим только доехавшее до main). `gh release create --generate-notes` собирает авто-ченджлог с прошлого релиза (диапазон между тегами; PR-ы, влитые с dev, попадают сюда же). Версия = тег, `package.json` НЕ бампаем. Дисциплина тегов — ручная: не выпускать/двигать теги без нужды, иначе диапазон «с прошлого релиза» посчитается криво.
