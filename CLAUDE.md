...
- **Никаких сторонних логгеров** — `console.log/warn/error` достаточно. Префиксы `[warn]`, `[presence]`, `[scheduler]` — соблюдать.
...
`.env` обязателен: `API_ID`, `API_HASH`, `BOT_TOKEN`, `ALLOWED_CHATS` (через запятую), опционально `DATA_FILE` (по умолчанию `./data.json`). Сессия mtcute — `bot.session` в CWD.
