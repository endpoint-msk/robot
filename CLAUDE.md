...
src/hosting.ts      — модель хостинга: dayKey/недели (пояс спейса), createHostingRequest, update/delete (dev), attendeesForDay (публичный «кто придёт»), архив, рассылка уведомлений, buildVisitIcs (.ics для календаря)
...
- **Перенос времени** (`proposeTime`/`acceptTimeProposal`/`clearTimeProposal`): пинг-понг вокруг `request.timeProposal` (`{ time, by: 'resident'|'guest', user, at }` или null). Сторона-адресат принимает (`proposal.accept` — согласованным становится `request.time`, предложение снимается) либо снимает предложение (`proposal.decline` — время не меняется). «Захостить» гасит висящее предложение.
  ...
  - `propose` со временем, **равным уже согласованному `request.time`**, — no-op: стейт не меняется, DM не уходит, в ответ обычный bootstrap. Иначе заводилось бы пустое предложение «перенести на 15:00» при времени 15:00. Проверка идёт **после** встречного согласия — там то же равенство означает другое. Фронт гасит это ещё до запроса (`screenVisit`, `proposeTimeFor`).
  - `propose` тем же временем, что **уже висит в твоём же предложении** (`counter.by === by && counter.time === time`), — тоже no-op: предложение не меняется, повторный DM второй стороне не шлём.
