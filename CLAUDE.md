...
- **Что показывает** — ближайший день с активностью в окне `[сегодня; +6]` (`activeDayForBoard`). Активность = есть подтверждённый визит **или** отметка резидента «я приду» (`dayHasActivity`; pending-заявки сами доску не заводят). Если сегодня пусто, а завтра есть активность — доска показывает завтра; появилась активность на сегодня — переключается на сегодня. Содержимое — публичный список `attendeesForDay` (тот же, что в bootstrap: резиденты «я приду» + подтверждённые неанонимные гости) + общее число заявок дня.
...
- **Мут** — `/boardmute` (админ чата, toggle, `state.hostingBoardMuted[chatId]`) отключает доску в чате: `syncChatBoard` в замьюченном чате открепляет и забывает висящую доску и новую не постит. Зеркалит `/goalsmute`/`/announcemute`/`/insidemute`.
