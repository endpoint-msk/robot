...
- **Кто придёт** (`attendeesView`, поле `days[].attendees` — видно всем, включая гостей): подтверждённые (`approved`) гости **без анонимных** + резиденты, отметившиеся «я приду». Резиденты идут первыми (приоритет) и с пометкой `resident:true`. Цель визита в публичный список НЕ попадает. Гость включает анонимность (`request.anon`) при создании/правке — резиденты в своих списках (`requests`) видят его всегда, с меткой «инкогнито».
...
- **Перенос времени** (`proposeTime`/`acceptTimeProposal`/`clearTimeProposal`): пинг-понг вокруг `request.timeProposal` (`{ time, by: 'resident'|'guest', user, at }` или null). Резидент предлагает другое время на любой pending-заявке (API `propose`), гость отвечает своим временем **только в ответ** на предложение резидента (тоже `propose`). Сторона-адресат принимает (`proposal.accept` — согласованным становится `request.time`, предложение снимается) либо снимает предложение (`proposal.decline` — время не меняется). «Захостить» гасит висящее предложение. Уведомления адресные: резидент→гость по `guest.userId`, гость→резидент по `user.userId` из прошлого предложения (для встречного гостя другого адреса нет — при самоотзыве DM не шлём). У одобренной заявки время не двигаем (`bad_status`).
