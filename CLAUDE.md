...
- **Никаких сторонних логгеров** — `console.log/warn/error` достаточно. Префиксы `[warn]`, `[presence]`, `[scheduler]`, `[printer]` — соблюдать.
...
`.env` обязателен: `API_ID`, `API_HASH`, `BOT_TOKEN`, `ALLOWED_CHATS` (через запятую), опционально `DATA_FILE` (по умолчанию `./data.json`), `PRINTER_URL` (Moonraker; без него `/printer` отключён). Сессия mtcute — `bot.session` в CWD.
